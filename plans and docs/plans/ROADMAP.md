...
**Roadmap correction (2026-06-06):** hand-tuned procedural noise proved structurally incapable of believable landforms ("Perlin oatmeal" → "mesa cliffs", the attempt #1–12 failure). M2 shape was replanned twice: (1) DEM distillation — an offline tool extracts each archetype's fingerprint (DONE, kept); (2) the spectral octave-sum *synthesis* was tried and abandoned (a global octave-sum makes uniform terrain, can't place discrete ranges). **The runtime synthesis is now WG10-informed LAYERED COMPOSITION:** a shared composition machine (domain warp, range envelope, ridged fbm, valley carve, blend) + per-biome RECIPE functions, tuned by the DEM fingerprints, biome-selected, border-blended. Each biome is its own structured recipe — a multi-recipe effort built **one gated recipe at a time** (PROVE WITH 3 first: mountain → grassland → desert). Sub-steps: M2.3 distill (done) → M2.4a machine+mountain → M2.4b grassland → M2.4c desert → (then remaining recipes) → M2.5 border blend → M2.6 perf pass LAST. See `M2_DESIGN.md` and the spec `docs/superpowers/specs/2026-06-06-m2-terrain-composition-machine-design.md`.
