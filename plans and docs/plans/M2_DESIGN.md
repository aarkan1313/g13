...
- **M2.3 ✓ BUILT (test gate PASS; visual gate PARKED 2026-06-06).** height = shared base landform + biome.detail_amp * detail_fbm(biome.detail_rough). Test gate m2_3_shaping_check PASS (rugged biome is roughest + 2.6x flattest, no cliff, deterministic). Visual (mountains rugged/plains flat) PARKED — needs LOW-altitude flight. M2.4 blends detail params across borders. DECISIONS (were locked, now built):
