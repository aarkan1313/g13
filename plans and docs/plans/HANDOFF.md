...
- **NEXT STEP: M2.1 — temperature & moisture climate fields (PLANNED, decisions locked; implement now).** READ `M2_DESIGN.md` FIRST — it's the plain-language explanation + the decisions already made, so you build without re-deciding: (1) Earth-like climate (temp ~ latitude − altitude + low-freq noise; moisture ~ low-freq noise), (2) climate produced in the SAME field dispatch as height (page carries height+temp+moisture — one source of truth), (3) viz = recolor terrain via a view-mode toggle (key V: normal/temp/moisture). M2.1 concretely touches field_height.glsl (add climate, page → ~3 channels), field_gpu.rs/page_pool.rs (carry the channels), ring_displace.gdshader (view_mode tint), world_view.gd (cycle V), + new `m2_1_climate_check.gd` gate (determinism + smooth/low-freq + range). Climate sampled in WORLD coords (00 §5), never page-local. Then M2.2 (Whittaker biome id) … see MILESTONE_2 / M2_DESIGN for the step list. The user wants "the best realistic system" and trusts the pillar-driven calls — apply them, explain in plain language as you build (they're learning the field internals), park the visual gate for their eyes.
