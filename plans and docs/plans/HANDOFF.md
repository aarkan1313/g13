...
## 3. Current state (update this in every handoff refresh) - refreshed 2026-06-06 (M2.4b structural scaffold planning)
...
- **CURRENT / NEXT STEP: M2.4b - DEM STRUCTURAL SCAFFOLD.** Use plan `docs/superpowers/plans/2026-06-06-m2-4b-dem-structural-scaffold.md`. The WG10 mountain synthesis lesson is: explicit range/channel/pass facts first, height assembly second, visual promotion last. Step 2/3 prototype now exists in `rust/structural_scaffold`: deterministic CPU-only RegionFacts plus `wg-13/_captures/m2_4b_scaffold_review.png/.md`; `cargo test -p structural_scaffold` is green. After the first bad toy-line sheet, the prototype was rewritten toward the actual WG10 recipe shape: domain-warped range envelopes, massif/base fields, primary+tributary carve masks, pass floors, and ridged/detail residuals. It is NOT runtime-integrated or visually accepted yet. Next: compare/tune the sheet against WG10 references, then add a candidate runtime lane separate from the accepted M2.3 baseline.
