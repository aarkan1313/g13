...
- **Levels do NOT overlap (annulus clipmap).** Each level draws ONLY the ring-shaped region (annulus) that the next-finer level does not cover. Two levels never render the same world ground at once — so there is no z-fighting between resolutions *by construction*. (Discovered in M1.5c: drawing fine+coarse on top of each other with a small Y bias z-fights badly — `render_priority` doesn't stop opaque depth-fighting. The annulus structure is also exactly what M1.6's 30 km LOD needs, so it's built once here.) Never-black still holds: where a fine page in the inner ring is missing, the coarse annulus is widened inward to cover the hole until the fine page loads.
