...
HUMAN VERDICT (the honest gate): "Not terrible. We will definitely want far greater diversity, better scale, and who knows -- but it's gonna take a while and work." So: the ARCHITECTURE is accepted as the baseline; the LOOK is a first pass. This is the right read -- terrain shape is the project's historically hard problem; one prototype pass doesn't finish it.
WHAT "GOOD" STILL NEEDS (the long work ahead, NOT done -- next effort's targets):
  1. FAR GREATER DIVERSITY -- only 6 archetypes, each a basic procedural recipe. Needs more archetypes (badlands, canyon, rolling-steppe, volcanic, coastal, fjord, dunes-proper...) AND richer character within each (the plains read dune-ish; recipes are first-pass fbm/ridged sums).
  2. BETTER SCALE -- region sizes + feature scale need work; the macro bands + archetype frequencies are untuned for a believable sense of scale across the journey.
  3. (open) richer per-region detail, transitions, possibly erosion (M6) to carve the relief into realism, water (M5) in the basins/swamps -- user wants M5/M6 pulled earlier (see below).
PLAN STATE: docs/superpowers/plans/2026-06-07-m2-5b-regional-archetype-terrain.md Tasks 1-3+5 DONE; Tasks 4 (tune archetypes), 6 (tune lone-peaks), 7 (acceptance+docs) are the VISUAL-tuning loop -- partially exercised (this verdict). The diversity/scale work is BIGGER than the plan's Task 4 tuning; it likely warrants its own brainstorm ("what archetype roster + scale make the journey read AAA") rather than just nudging the existing 6 recipes. field_height_probe.glsl still exists (superseded by production; can delete or keep as reference).
NEXT (user-directed): after terrain shape matures, REVIEW pulling M5 (water) + M6 (erosion) earlier -- erosion carves the new relief into realism, water settles in valleys/swamps. That review is its own brainstorm.
CODEBASE STATE: dem-grounded branch, clean. Production = regional-archetype terrain, 100km reach, Rust streaming + floating origin. All gates green. A solid BASELINE to build the diversity/scale work on.
...
THE 120fps-DIP MICRO-PASS (user: "wonder if there's perf fixes we're missing"). ATTRIBUTION PROBE (per-system breakdown on worst frames) found the dip is NOT GPU/streaming (produce_us = 0 even while producing 4 pages/frame — M2.6 GPU-resident did its job) but the VIEW'S PER-FRAME GDSCRIPT: it re-scanned all (2r+1)^2 ring cells for EVERY level EVERY frame (string-formatting keys + dict lookups) just to find the usually-zero missing pages — 1.6ms even STATIONARY, 6ms at the dip.
  ...
  STEP B (committed) — ANNULUS STRING-KEY KILL: _update_annulus_visibility rebuilt a redundant `displayed` dict + did ~366x4 "%d:%d:%d" string-format lookups/frame. Replaced with ONE int64-packed resident set (_pack_key(level,gx,gz)) + integer-hash footprint checks; same annulus logic, never-black intact (smoke: no pin_violation/invalid). Measured: stationary process 974->499us; dip total 6.94->6.28ms (~144->159fps). CUMULATIVE 8.31->6.28ms (~120->159fps).
  STOPPED HERE (pillar call): the dip's REMAINING moving cost is the per-frame PIN pass (366 _pool.pin_page FFI calls) + the fine-level request scan. Pinning is per-frame BY CONTRACT (page_pool begin_frame does pinned.clear(); the view re-asserts what's displayed) -> can't make it incremental without a Rust contract change (rebuild + eviction-safety risk). Converting _instances to int keys would touch many call sites. Both are structural; shaving ~1ms off a frame already at 2.6x budget (159fps) isn't worth that risk when the user said "good enough." Bank it. If perf ever regresses toward 60, the next levers (in order): int-key _instances; then the Rust per-frame-pin contract.
  STILL TO DO: PORT the whole perf pass (config restore + Step A skip + Step B annulus) to production world_view.gd (same hot loop, same wins available). Not yet done.
...
DID NOT PROCEED to P2. CODEBASE STATE: green at d56220c + Codex's inert uncommitted rust edits (untouched).
...
MEASURED (low-fly A/B, same probe, same machine): avg frame 8.02 -> 7.06 ms; produce_us 2250 -> 1717 us/frame (-24%). Burst gate still 0/720 over budget. Full 10-gate suite GREEN (m1_4/m1_5c/m1_7a/m1_7b/m1_7c/m2_1/m2_2/m2_3/m2_6_vram/m2_6_burst).
NET PERF for the whole M2.6 pass: streaming BURST 47ms -> 13ms median-of-maxes (0/720 over the 16.6 budget); low-fly ~7ms; the 609-render-errors/frame use-after-free bug -> 0. The world streams ~as smooth as stationary now. User re-fly earlier: "better, more stable on average."
HUMAN GATE: launched for a final test-fly at session end (perf should feel smoother than the prior try). Result to be confirmed in the NEXT chat — if it still feels heavy, the next levers are: batch the MAIN-device render dispatch too (smaller gain, no blocking sync there), or lower max_new_per_frame. But perf is in a strong banked place.
PRESERVED: Codex's PagePool.cpu_readback_enabled toggle (DEM-review escape hatch to skip collision in the visual-only DEM scene). NOT accepted production architecture (user) — fine to rework/discard later; batching respects it.
NEXT CHAT (priorities): (1) confirm the test-fly verdict; (2) the DEM-GROUNDED terrain direction is the user's chosen big track — they have an active prototype: untracked wg-13/scripts/dem_grounded_*, wg-13/shaders/field_height_dem_grounded.glsl, wg-13/scenes/dem_grounded_terrain_review.tscn, docs/superpowers/specs/2026-06-07-dem-grounded-procedural-terrain-architecture.md (Codex's). Fold that into the DEM-direct brainstorm. (3) M2.5 visual-acceptance was skipped by the out-of-order perf work — still pending. Optional: M2.6-more (render-dispatch batch).
