...
M2.4-reframe [ ] <- CURRENT. BRAINSTORM the terrain direction before code (macro resolution / detail-above-macro / viewing-LOD model / surface-in-per-cell / or M2.3-is-enough -> M3 materials). Reusable in git: DEM kernel library, macro bridge, window-port (SHAs in postmortem).
