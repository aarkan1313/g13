...
M2.3 [x] per-biome height shaping (detail-on-shared-base; no circularity; data-driven detail_amp/rough). Test gate PASS (m2_3_shaping_check). Live flyover (human): the underlying SHAPE read as "Perlin oatmeal" — plain value-noise fBM is structurally incapable of real landforms (the attempt #1-12 failure). Decision: insert M2.3b (real macro-landform noise) before continuing; M2.3's per-biome mechanism is reused, the BASE noise is what's replaced.
M2.3b [ ] REAL MACRO-LANDFORM FIELD (roadmap insertion 2026-06-06). Replace plain fBM base with domain-warped + ridged macro-elevation: actual ridgelines/valleys, organic non-blobby continental structure. Per-biome = ridge intensity (mountains sharp, plains smooth) on the shared continuous base (no cliff, no circularity preserved). The PERMANENT shape infra: erosion M6 carves into it, DEM-stats M2.5/6 tune it, textures M3 drape it, scatter M4 reads slope. High-freq detail kept MINIMAL (erosion's job, not throwaway here). GATE (test): ridge structure exists + per-biome + deterministic + no cliff. GATE (visual): reads as real terrain. Spec + design committed.   <- CURRENT
M2.4 [ ] border blending, no hard square borders
M2.5 [ ] offline DEM-stats tool outputs params file (redefined: stats TUNE the M2.3b ridged macro field, not rescue placeholder noise)
M2.6 [ ] DEM-tuned biome shape reads more believable than hand-tuned ridged (feeds DEM stats into M2.3b per-biome ridge/warp params)
