...
M2.4 [ ] DEM STRUCTURAL SCAFFOLD (M2.4b): RETHINK after M2.4a visual fail. The scalar DEM-character approach is backed out; it passed numeric gates twice but failed live visual review twice with corduroy grooves / harsh parallel walls. Main is back to the M2.3 visual-pass terrain; failed M2.4a is preserved at branch/tag `backup/m2-4-failed-2026-06-06`. New direction: adapt the WG10 mountain synthesis lesson procedurally, not as a static baked world. Build deterministic Rust-owned region facts (range/ridge/channel/pass/material hints) from seed+region, use GPU where dense macro fields belong, cache/sample those facts in runtime pages, and let climate/biome/elevation bias style/materials without becoming hard terrain recipe branches. Plan: `docs/superpowers/plans/2026-06-06-m2-4b-dem-structural-scaffold.md`. Step 2/3 prototype landed then revised: `rust/structural_scaffold` now carries an adapted WG10 Rust recipe port (`recipe_noise`, `array_ops`, mountain seam-safe synthesis) and renders the 200 km / seed 177 four-style review sheet to `wg-13/_captures/m2_4b_scaffold_review.png/.md`; static macro 3D review scene at `wg-13/scenes/m2_4b_scaffold_3d_review.tscn` demonstrates promising structure but is too compressed/unplayable as gameplay scale; scale-calibration scene added at `wg-13/scenes/m2_4b_scaffold_playable_scale_review.tscn`; smoke PASS covers both rendered scenes (macro 4 panels / 148996 vertices, playable 1 panel / 37249 vertices); reproduction record: `docs/superpowers/plans/2026-06-06-m2-4b-wg10-port-record.md`; unit/workspace gates PASS. THEN (2026-06-06) the GPU-portable per-cell oracle (`sample_cell`) was rendered for the first time: `oracle_fact_map` + `--source window|oracle` CLI flag (window output byte-identical) + `m2_4b_scaffold_oracle_playable_review.tscn` + gate coverage (3 scenes PASS, exit 0). Plan: `docs/superpowers/plans/2026-06-06-m2-4b-oracle-review-bite.md`; spec: `docs/superpowers/specs/2026-06-06-m2-4b-oracle-validation-and-integration-design.md`. Static review INCONCLUSIVE -> integrated the oracle into live `demo.tscn` as a toggleable candidate lane (B key, terrain_mode 0=REFERENCE/1=SCAFFOLD_CANDIDATE; full plumbing + gate `m2_4b_oracle_live_check` PASS + all regression gates green; std430 stayed 80 bytes). WALK-TEST VERDICT (2026-06-07): persistent dark crater/walls + flat/wrong shape. ROOT CAUSE (measured): oracle coarse-LOD max step 1076m vs M2.3's 482m — its sharp analytic channel/carve features alias catastrophically at coarse LOD (1km walls between cells/levels); a per-cell field can't do the Gaussian neighbor-smoothing that made the WG10 window-port read well. Same family as the M2.4a corduroy failure. DECISION (human-confirmed Branch 3): ESCALATE to **Approach C** = window-based GPU pipeline (apron + multi-pass blur + flow routing across pages, seam-safe at every LOD). The oracle candidate lane STAYS (non-destructive base layer C builds on; toggle/param/collision plumbing reused). Next: brainstorm+spec Approach C as a milestone-scale step. See DRIFT_LOG top.   <- CURRENT (Approach C spec next)
