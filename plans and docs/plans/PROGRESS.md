...
M2.3 [~] COMPOSITION MACHINE: structure (primitives + uplift field + composition, hand-set character). Test gate PASS (m2_3_composition_check: determinism, structure-not-uniform spread 0.81, no-cliff 5.1m; m2_1/m2_2 still PASS). Captures show believable varied terrain (lowlands + ranges ~1.5km with valleys, ~2.2km relief span). Capture tool made ground-aware. VISUAL sign-off from human still PENDING (deep-dived into character-controller physics instead).   <- CURRENT (awaiting visual verdict)
