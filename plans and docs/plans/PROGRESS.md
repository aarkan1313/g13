...
M2.2 [~] Whittaker biome id (nearest-centroid over temp/moisture/MACRO-altitude), N-axis data-driven roster (10 biomes), field-side, page gains biome-id channel. Test gate PASS (m2_2_biome_check: determinism, valid ids [0,10), contiguity 0% adjacent-differ on fine pages, global variety, seed sensitivity — 15/15 gates green). Tuning during build: (a) macro-altitude = continental low-freq landform (NOT detailed height) -> killed confetti-at-LOD (38.8%->~9% at coarsest); (b) rebalanced M2.1 temp (warm floor + altitude-normalized lapse) -> all 10 biomes appear (was cold-skewed, temp mean 0.19->0.48). VISUAL gate (large contiguous regions, no confetti) PARKED-FOR-VISUAL: capture _captures/climate_biome.png; V cycles to biome mode in the live scene.   <- CURRENT (awaiting human visual confirm; M2.3 next)
