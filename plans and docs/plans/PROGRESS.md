...
M2.4-SPECTRAL-ABANDONED [~] spectral octave-sum synthesis. Built + test-gated, but VISUAL failed: an amplitude spectrum applied as a global octave-sum makes UNIFORM terrain (spiky oatmeal at fine scale; gentle rolling everywhere when remapped) — it can't place a discrete mountain RANGE with valleys. Root-caused; reverted the synthesis. (Rust fingerprint loader + per-biome GPU table KEPT — they tune the new approach.)
   M2 TERRAIN REDESIGNED (2026-06-06) around WG10's proven mountain technique: LAYERED COMPOSITION (base + range_envelope x ridge_detail - valley_carve), as a shared composition MACHINE + per-biome RECIPE functions, DEM-tuned, biome-selected, border-blended. Each biome = its own structured recipe. Spec: docs/superpowers/specs/2026-06-06-m2-terrain-composition-machine-design.md. DECISION: PROVE WITH 3 RECIPES FIRST (mountain, grassland, desert), then expand.
M2.4a [ ] COMPOSITION MACHINE + MOUNTAIN RECIPE (+ fallback recipe for other biomes). Machine = shared GLSL primitives (domain warp, region envelope, ridged fbm, valley carve, blend), DEM-tuned. Mountain recipe = envelope x ridges concentrates relief into ranges with valleys between; relief from slope_p95. GATE (test): determinism, range-structure (high-envelope tall / low-envelope flat, not uniform), no cliffs. GATE (visual): fly low over a mountain region -> reads as real ranges/valleys, NOT oatmeal. THE MAKE-OR-BREAK.   <- CURRENT
M2.4b [ ] GRASSLAND recipe (flat plains, gentle). Gated like M2.4a.
M2.4c [ ] DESERT recipe (dune fields). Gated. After these 3 prove the pattern -> schedule the rest.
...
M2.5 [ ] border blending (refine the recipe-blend if needed). GATE (visual): natural transitions.
M2.6 [ ] EFFICIENCY/PERF PASS (LAST, the M1.9 way): profile the REAL composed field vs the frame budget. GATE (test): budget held on the RTX 3070 target.
