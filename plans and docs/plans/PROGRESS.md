...
M2.4 [~] DEM-INFORMED TERRAIN SHAPE — the long bite. Attempts a->c each FAILED visual review but each taught a root-caused lesson and left REUSABLE infra. Full narrative lives in DRIFT_LOG; the clean state is the sub-lines below. The accepted-live terrain is STILL M2.3 (REFERENCE, mode 0); the macro lane (mode 2) renders but its CHARACTER is wrong. CURRENT = M2.4d (make the macro DEM-driven).
  ...
  M2.4d [~] DEM-DRIVEN TERRAIN — built the foundation, TRASHED the bake on visual review (2026-06-07). The DEM kernel pipeline works MECHANICALLY (real-DEM character extracted, blended, applied; A/B probe proved mode-2 differs from REFERENCE by 564m/2978m) but the live look is still BLOCKY/terraced/bad. ROOT CAUSE (the lesson): the **256m macro grid itself is the visible artifact** at altitude/coarse-LOD — character source (procedural vs DEM) is the WRONG LAYER; macro RESOLUTION × LOD × VIEWING SCALE is the real problem. COMMITTED + REUSABLE (kept on main): spec `docs/superpowers/specs/2026-06-07-m2-4d-dem-driven-terrain-design.md`, plan, km-scale ramp, structure-gated detail, the DEM kernel SPIKE + curated asset `wg-13/data/dem_kernels.bin` (32/arche @128^2, 4MB) + the `macro_cache::kernels` loader/blender. TRASHED + reverted: the DEM-driven `MacroBake` (Phase B Task 6). Live terrain back to M2.3 REFERENCE (known-good). See DRIFT_LOG top.
  M2.4e [ ] <- CURRENT. **M2 TERRAIN NEEDS A FRESH REVIEW** (human-called). Resolve the macro-resolution / LOD / viewing-scale question BEFORE more code — the cached-256m-macro (Approach C) may have a visual ceiling at altitude. The DEM kernel library is a reusable character SOURCE for whatever architecture the review picks. Do NOT just re-tune the DEM bake (re-polishing under the same ceiling). Brainstorm/review first.
