...
M2.4-perf [ ] <- CURRENT. The seam fix added 4 extra composition_height evals/cell (analytic-normal neighbor taps) -> ~5x the height math in page PRODUCTION (shows as higher prod-ms during streaming/motion, not steady-state). User confirmed the regression ("not performant anymore but now we can build that") and chose SHIP-then-PERF. Fix options (profile-first, M1.9 way): (a) analytic derivative inside composition_height (one pass, no neighbor re-eval — fastest); (b) share octave work across the 5 taps; (c) drop high-freq detail octaves from the NORMAL taps only (normal reads smooth without them). Real gate = prod-ms back near baseline on the RTX 3070 target with seam still gone.
