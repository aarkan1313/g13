...
- **M2.3** — Per-biome height shaping wired into the field. GATE (visual): mountains are mountainous, plains are flat, visibly different terrain per biome.
...
1. **Slope/roughness statistics (easiest, do first):** From a DEM of, say, a mountain range, measure the distribution of slopes and the typical feature spacing (via frequency analysis). Tune the biome's noise octaves to match that distribution. Result: your procedural mountains have the *texture of realness* of real mountains. This is just parameter-fitting — low risk, high payoff.
...
- **M2.5** — Offline DEM-stats tool produces slope/roughness params for 2–3 reference terrain types. GATE (test): tool runs, outputs a small params file.
- **M2.6** — Mountain/hill biomes use DEM-derived params. GATE (visual): side-by-side, DEM-informed terrain reads as more believable than the pure-noise version. (Human visual call.)
